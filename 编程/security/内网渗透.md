...
```
网络基础(2周) → 信息收集(2周) → 漏洞利用(3周) → 权限提升(2周) → 横向移动(3周) → 持久化(1周) → 防御对抗(2周) → 实战演练(持续)
```

## ⚖️ 法律声明与道德准则

> **严正声明**: 内网渗透必须在获得明确书面授权的前提下进行！
...
```bash
# CIDR表示法
192.168.1.0/24    # 256个IP (254可用)
10.0.0.0/8        # 16,777,216个IP
172.16.0.0/16     # 65,536个IP
...
**VLAN原理**:
```
VLAN 10: 财务部门 (172.16.10.0/24)
VLAN 20: 研发部门 (172.16.20.0/24)
VLAN 30: 运维部门 (172.16.30.0/24)
VLAN 99: 管理VLAN (172.16.99.0/24)
```
...
# 利用unquoted服务路径
# 服务路径: C:\Program Files\Some Service\service.exe
# 尝试创建: C:\Program.exe 或 C:\Program Files\Some.exe
...
# MS15-051（Win7/8/Server 2008/2012）
use exploit/windows/local/ms15_051_client_copy_image

# MS14-058（Win7/8/Server 2008/2012）
...
**reGeorg**:
```bash
# 1. 上传tunnel脚本（tunnel.aspx/jsp/php）
# 2. 启动代理
...
```bash
meterpreter > ps
meterpreter > migrate 1234  # 迁移到合法进程
```
...
### ✅ 阶段二：技能提升（3-4个月）
- [ ] 掌握5种以上横向移动技术
- [ ] 能够进行Kerberos攻击（Kerberoasting、Golden Ticket）
- [ ] 熟练使用Mimikatz、BloodHound等工具
- [ ] 编写自动化渗透脚本
- [ ] 完成域渗透CTF挑战
...
### 在线靶场
- **HackTheBox**: Active Directory靶机
- **VulnHub**: 内网渗透系列靶机
- **GOAD**: 游戏化AD环境
- **PentesterAcademy**: 红队实验室
...
**靶场推荐配置**:
```
1台域控 (Windows Server 2019)
2台域成员机 (Windows 10)
1台Web服务器 (Ubuntu + DVWA)
1台数据库服务器 (Windows + MSSQL)
1台Kali攻击机
```
...
1. **《内网安全攻防》** - 系统化内网渗透知识
2. **《渗透测试实践指南（第3版）》** - 经典必读
3. **《Active Directory安全指南》** - AD安全深度解析
4. **《PowerShell攻防实战》** - PowerShell渗透技巧
...
1. **法律第一**: 所有技术仅用于授权测试
2. **保密义务**: 严格保护客户数据和测试结果
3. **最小影响**: 避免对生产环境造成破坏
4. **及时沟通**: 发现高危漏洞立即报告
5. **持续学习**: 攻防技术不断演进，保持学习状态

---

## 📝 总结

内网渗透是信息安全领域的高级技术方向，需要扎实的基础知识和大量的实践经验。本指南提供了系统化的学习路径，但真正的技能提升需要：

1. **深入理解**: 不仅知道怎么做，更要理解为什么
2. **大量实践**: 理论必须结合靶场和实战演练
3. **举一反三**: 从单个技术扩展到完整攻击链
4. **对抗思维**: 理解蓝队防御，提升红队能力
5. **道德底线**: 技术是为了安全防御，而非恶意破坏

记住：优秀的内网渗透工程师不仅是技术专家，更是具有职业道德和法律意识的安全从业者。
